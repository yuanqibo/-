...
function normalizeEcpLegacyUserPayload() {
...
function upsertEcpUser(user) {
  const matched =
    ...
    state.users.find((item) => item.account === user.account);
...
function applyEcpLegacySession() {
  if (!isEcpAuthEnabled()) return false;
  const ecpUser = normalizeEcpLegacyUserPayload();
  ...
  state.route = readPersistedRoute() || preferredAccessibleRoute();
  persistRoute(state.route);
  return true;
}

window.assetPortalApplyEcpSession = applyEcpLegacySession;
window.addEventListener("asset-portal-ecp-session", () => {
  if (applyEcpLegacySession()) render();
});
...
function render() {
  if (!isAuthenticated() && isEcpAuthEnabled() && applyEcpLegacySession()) {
    render();
    return;
  }
...
async function bootApp() {
  ...
  if (isEcpAuthEnabled() && applyEcpLegacySession()) {
    render();
    return;
  }
