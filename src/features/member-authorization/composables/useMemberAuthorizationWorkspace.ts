...
export const useMemberAuthorizationWorkspace = () => {
  ...
  const state = reactive<MemberAuthorizationWorkspaceState>({ loaded: false, docked: false, overlayOpen: false, errorMessage: '' })
  ...
  let observedOverlayOpen = false
  let observedWorkspaceHidden = false
  ...
  const positionDock = (): void => {
    if (!dockedShell || !dockAnchor?.isConnected || dockedShell.classList.contains('is-workspace-drawer-open')) return
  ...
  const setOverlay = (active: boolean, hideWorkspaceApp = active): void => {
    if (state.overlayOpen !== active) state.overlayOpen = active
    ...
    try {
      ...
      // The embedded workspace is same-origin in supported deployments.
    }
  ...
  const disconnectObserver = (): void => {
    ...
    drawerObserver = null
    cancelScheduledSync()
    observedOverlayOpen = false
    observedWorkspaceHidden = false
  }
  ...
  const observeFrame = (): void => {
    ...
    try {
      ...
      const sync = (): void => {
        ...
        const overlayOpen = activeOverlays.length > 0
        const workspaceHidden = activeOverlays.some((overlay) => !overlay.closest('#app'))

        if (overlayOpen === observedOverlayOpen && workspaceHidden === observedWorkspaceHidden) return
        observedOverlayOpen = overlayOpen
        observedWorkspaceHidden = workspaceHidden
        setOverlay(overlayOpen, workspaceHidden)
      }
  ...
  const requestWorkspace = (element: HTMLIFrameElement): void => {
    ...
    loadTimer = window.setTimeout(() => {
      ...
      // Function refs run before Vue finishes inserting the surrounding subtree.
      // Dock only after that mount pass so Vue does not move the shell back into the panel.
      dockFrame(element)
  ...
  const frameLoaded = (): void => {
    ...
    try {
      ...
      // A deployment origin mismatch is reported by observeFrame below.
    }
  ...
  const disconnect = (): void => {
    ...
    dockedShell?.classList.remove('is-workspace-docked', 'is-workspace-drawer-open')
    dockedShell?.style.removeProperty('--authz-workspace-dock-left')
    dockedShell?.style.removeProperty('--authz-workspace-dock-top')
    dockedShell?.style.removeProperty('--authz-workspace-dock-width')
    dockedShell?.style.removeProperty('--authz-workspace-dock-height')
    dockedShell?.remove()
    dockedShell = null
    dockAnchor = null
    state.docked = false
