...
import type { CatalogNode } from '../../assets/types/assets'
...
const isDecisionSyncing = (item: ApprovalRecord): boolean => Boolean(item.decisionSubmitted) && ['审批中', '待执行'].includes(item.status)
...
onMounted(async () => {
  ...
  const requestType = route.query.action === 'return' ? '资产退还' : route.query.action === 'handover' ? '资产交接' : ''
  if (!requestType || !isEmployeeTerminal.value) return
  openRequest(requestType)
  const assetId = typeof route.query.asset === 'string' ? route.query.asset : ''
  if (assetId && requestAssets.value.some((item) => item.id === assetId)) requestForm.assetIds = [assetId]
})
</script>
