    ...
    <section v-else class="asset-label-template-page">
      ...
      <div class="asset-label-template-right"><form class="asset-label-template-config-form first-template-config-form" @submit.prevent="saveLabel">
        ...
        <div class="asset-label-template-stage">
          ...
          <div v-else class="access-label-config-preview" :style="labelPreviewStyle" :aria-label="`配置1 ${labelSettings.labelWidth}*${labelSettings.labelHeight}mm 预览`"><div class="access-label-preview-card"><div class="access-label-preview-qr"><AssetQrGraphic text="资产编码:X" /></div><strong class="access-label-preview-code">X</strong><strong class="access-label-preview-name">X</strong><span class="access-label-preview-owner">{{ String(labelSettings.ownershipText || '此资产归Access集团所有') }}</span></div></div>
        </div>
