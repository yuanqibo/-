...
const templateOptions = computed(() => [
  ...
  ...customTemplates.value
    .filter((item) => String(item.baseTemplateKey || (item.settings as Record<string, unknown> | undefined)?.templateKey || '') !== 'access')
...
const syncFromStore = (): void => {
  ...
  const legacyAccessTemplate = storedTemplateKey === 'access'
  // A removed template must not leave its sizing and fields behind when falling back to the standard template.
  Object.assign(labelSettings, clone(builtInTemplates[0].settings), { templateKey: 'standard' }, legacyAccessTemplate || storedTemplateKey === 'defaultAsset' ? {} : storedLabelSettings, { templateKey })
}
