...
const aliases: Record<keyof AssetDraft | string, string[]> = {
  ...
  location: ['所在位置', '资产位置', '位置', 'location'],
...
const rowDraft = (headers: string[], values: string[], rowNumber: number): AssetImportRow => {
  ...
  const errors: string[] = []
  if (!raw.name) errors.push('资产名称不能为空')
  if (!raw.category) errors.push('资产分类不能为空')
  if (!raw.location) errors.push('所在位置不能为空')
  const price = Number(raw.price || 0)
  if (!Number.isFinite(price) || price < 0) errors.push('金额格式不正确')
  const draft = errors.length ? null : {
    ...raw,
    name: raw.name,
...
export const parseAssetWorkbook = async (file: File): Promise<AssetImportRow[]> => {
  ...
  return rows.slice(1).map((values, index) => rowDraft(rows[0], values, index + 2)).filter((row) => row.draft || row.errors.length)
}
