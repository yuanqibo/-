...
test.describe('登录后门户质量回归', () => {
  ...
  test('资产处置支持退租、部分取消与完成流转', async ({ page, isMobile }) => {
    ...
    const disposalSubmit = dialog.getByRole('button', { name: '保存并提交', exact: true })
    await expect(disposalSubmit).toBeDisabled()
    await dialog.getByLabel('选择DISP-LX-1').check()
    await dialog.getByLabel('选择DISP-OWN-2').check()
    await expect(disposalSubmit).toBeEnabled()
    await disposalSubmit.click()
  ...
  test('资产列表可在当前页直接发起处置', async ({ page, isMobile }) => {
    ...
    const disposalSubmit = dialog.getByRole('button', { name: '保存并提交', exact: true })
    await expect(disposalSubmit).toBeDisabled()
    await dialog.getByLabel('选择DISP-DIRECT-1').check()
    await expect(disposalSubmit).toBeEnabled()
    await disposalSubmit.click()
  ...
  test('资产退还或编辑成功后回到未筛选的资产主列表', async ({ page, isMobile }) => {
    ...
    const returnSubmit = returnDialog.getByRole('button', { name: '保存并提交', exact: true })
    await expect(returnSubmit).toBeDisabled()
    await returnDialog.getByLabel('选择AST-CONTEXT-RETURN').check()
    await expect(returnSubmit).toBeEnabled()
  ...
  test('领用与借用表单保留资产明细和逐项日期', async ({ page, isMobile }) => {
    ...
    await picker.locator('tbody tr').filter({ hasText: 'AST-0003' }).locator('.el-checkbox').click()
    ...
    await page.getByRole('option', { name: '杭州仓库', exact: true }).click()
    const receiveSubmit = receiveDialog.getByRole('button', { name: '保存并提交', exact: true })
    await expect(receiveSubmit).toBeDisabled()
    await receiveDialog.getByLabel('选择AST-0002').check()
    await expect(receiveSubmit).toBeEnabled()
