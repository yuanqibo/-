...
test.describe('登录后门户质量回归', () => {
  ...
  test('成员授权窗口按 ECP 视口样式展示且切换菜单后不残留', async ({ page, isMobile }) => {
    ...
    await expect(frame.locator('.el-overlay')).toHaveClass(/authz-workspace-host/)
    ...
    expect(shellBox).toMatchObject({ x: 0, y: 0, width: viewport?.width, height: viewport?.height })
    ...
    await frame.getByRole('button', { name: '关闭账号授权', exact: true }).click()
  ...
  test('盘点与资产设置表单保留迁移前结构和层级逻辑', async ({ page, isMobile }) => {
    ...
    await labelPage.getByRole('button', { name: '选择默认资产标签', exact: true }).click()
    await expect(labelPage.locator('.default-label-editor-form')).toBeVisible()
    await expect(labelPage.getByText('模板、尺寸、字段与版面', { exact: true })).toBeVisible()
    await expect(labelPage.getByText('标签显示字段', { exact: true })).toBeVisible()
    await expect(labelPage.getByText('扫码显示字段', { exact: true })).toBeVisible()
